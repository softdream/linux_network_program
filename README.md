...
## 8. epoll + 非阻塞IO + 边沿触发
&emsp;epoll的非阻塞IO模式是利用边沿触发模式，同时修改read()函数模式为非阻塞读取，这样在减少epoll_wait()函数调用的同时也实现了水平触发的效果，是对epoll应用最优的模式。

## 9. epoll的反应堆模型
&emsp;Epoll一般的应用模型为：<br>
&emsp;a. 程序设置边沿触发以及每一个上述的文件描述符设置为非阻塞；<br>
&emsp;b. 调用epoll_create()创建一个epoll对象；<br>
&emsp;c. 调用epoll_ctl()向epoll对象进行删除、增加等操作；<br>
&emsp;d. 调用epoll_wait()返回待处理的事件集合；<br>
&emsp;e. 一次调用事件集合中的每一个元素中的event中的回调函数（通过函数指针）；<br>
&emsp;流程：监听可读事件（ET）->事件到来->触发事件->epoll_wait()返回->处理回调函数->继续epoll_wait()->直到这个线程结束之间一直重复这个循环；<br>
&emsp;Epoll反应堆模型流程为：<br>
&emsp;监听可读事件（ET）->事件到来->触发事件->epoll_wait()返回-><br>
&emsp;读取完数据（可读事件的回调函数内）->将该节点从红黑树上摘下（可读事件的回调函数内）-><br>
&emsp;设置可写事件和对应的回调函数（可读事件的回调函数内）->将可写事件挂上树（可读事件的回调函数内）-><br>  
&emsp;处理数据（可读事件的回调函数内）-><br>
&emsp;监听可写事件（ET）->对方可读->触发事件->epoll_wait()返回-><br>
&emsp;写完数据（可写事件回调函数内）->将该节点从红黑树上摘下（可写事件回调函数内）-><br>
&emsp;设置可读事件和对应的回调函数（可写事件回调函数内）->将可读事件挂上树（可写事件回调函数内）-><br>
&emsp;处理收尾工作（可写事件回调函数内）->直到这个线程结束之间一直重复这个循环；<br>
